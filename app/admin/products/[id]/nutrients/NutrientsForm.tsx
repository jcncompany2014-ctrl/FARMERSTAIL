...
export default function NutrientsForm({
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-7">
      {/* AAFCO 평가 카드 */}
      <div className="rounded border border-border bg-secondary p-4 text-sm">
        <div className="font-semibold text-foreground mb-2">AAFCO Adult Maintenance 평가</div>
      ...
      <div className="sticky bottom-0 bg-secondary py-4 border-t border-border flex items-center gap-3">
        <button
          type="submit"
          disabled={pending}
        ...
        {status && (
          <span
            className={`text-xs ${status.startsWith('오류') ? 'text-destructive' : 'text-emerald-700'}`}
          >
            {status}
          </span>
        )}
    ...
    <div>
      ...
      <div
        className={`text-lg font-semibold ${
          value === 0 ? 'text-emerald-700' : color === 'sale' ? 'text-destructive' : 'text-muted-foreground'
        }`}
      >
        {value}
      </div>
    ...
    <label className="block">
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          className={`flex-1 rounded border px-3 py-2 text-sm bg-secondary ${
            flag ? 'border-sale' : 'border-border'
          }`}
          placeholder="—"
        />
        <span className="text-xs text-muted-foreground w-14 text-right">{spec.unit}</span>
      </div>
      {spec.hint && (
        <div className="mt-1.5 text-xs text-muted-foreground leading-snug">{spec.hint}</div>
      )}
    </label>
  )
}

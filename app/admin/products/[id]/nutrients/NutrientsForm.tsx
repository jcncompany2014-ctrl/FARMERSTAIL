...
export default function NutrientsForm({
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-7">
      ...
      <div className="sticky bottom-0 bg-zinc-50 py-4 border-t border-zinc-200 flex items-center gap-3">
        <button
          type="submit"
          disabled={pending}
    ...
    <div>
      ...
      <div
        className={`text-lg font-semibold ${
          value === 0 ? 'text-moss' : color === 'sale' ? 'text-sale' : 'text-zinc-500'
        }`}
      >
        {value}
      </div>
  ...
  return (
    <label className="block">
      <div className="flex items-baseline justify-between mb-1.5">
        <span className="text-sm text-zinc-900">{spec.label}</span>
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          className={`flex-1 rounded border px-3 py-2 text-sm bg-zinc-50 ${
            flag ? 'border-sale' : 'border-zinc-200'
          }`}
          placeholder="—"
        />
        <span className="text-xs text-zinc-500 w-14 text-right">{spec.unit}</span>
      </div>
      {spec.hint && (
        <div className="mt-1.5 text-xs text-zinc-500 leading-snug">{spec.hint}</div>
      )}
    </label>
  )
}

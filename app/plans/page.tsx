 ...
 * 2주마다(화요일) 고정이고 고객이 고르는 건 화식 비율(30/50/100)뿐이라, 웹이
...
// 화식 비율(30 곁들임 / 50 반반 / 100 완전) — 앱 PlanClient FRESH_TIERS 와 동일한
...
const PLANS: Plan[] = [
  ...
  },
  {
    ...
    en: 'FULL 100%',
    cadence: '화식 100%',
    lede: '매일 그릇 가득 신선한 화식만. 완전한 영양을 원하는 아이를 위한 구성이에요.',
    bullets: ['모든 끼니를 신선한 화식으로', '수의영양 기준 100% 충족', '2주마다 급여량 리뷰·조정'],
  },
]

const WHY = [
  // 효능을 단정하지 않는다(표시광고법 §3 — 실증자료 없는 효과 단정 금지).
  // /why-fresh·/recipe 와 같은 어투로 '관찰 프레임'만 준다.
  { n: '01', t: '영양소의 일관성', b: '같은 수의영양 프로파일이 몇 주 이어지면 변·피부·체중 같은 일상 변화를 비교해 보기 쉬워요. 아이마다 차이가 있어요.' },
  { n: '02', t: '신선도', b: '주문이 확정된 만큼만 조리·냉동·출고합니다. 유통기한 기반 재고 관리와는 달라요.' },
  { n: '03', t: '합리적인 구조', b: '출고량이 예측 가능해, 같은 품질을 더 합리적으로 제공할 수 있어요.' },
  // '언제든' 은 쓰지 않는다(사장님 브랜드 보이스). 라벨은 실제 버튼과 같은 말로.
  { n: '04', t: '관리의 자유', b: '다음 결제 전에 미루거나 멈출 수 있어요. 구속하지 않는 구독이 좋은 구독이라고 생각해요.' },
]
...
const STEPS = [
  ...
  ['04', '2주마다 자동 출고 — 미루기·멈춤 자유'],
]

const FAQS = [
  // ★ 2026-07-30 정정. 세 가지가 틀렸다:
  //  ① '발송 1주일 전까지' — 저장소에서 **세 번째로 다른 마감**이었다(FAQ 는
  //     일요일, 여기는 1주일 전). 실제로 해지·미루기는 그 회차 **결제 전**이면
  //     반영된다(status 를 바꾸는 순간 청구 대상에서 빠진다).
  //  ② '비율 변경' — 기존 구독의 화식 양을 바꾸는 화면이 없다. DB 권한도
  //     회수돼 있다(20260730000000).
  //  ③ '언제든' — 사장님이 금지한 표현.
  { q: '해지가 자유로운가요?', a: '네. 다음 결제 전에 마이페이지에서 미루기·일시정지·해지를 하시면 그 박스는 나가지 않아요. 위약금이나 최소 약정은 없습니다.' },
  { q: '배송은 어떻게 오나요?', a: '드라이아이스 + 진공 단열재 박스로 냉동 상태를 유지해, 2주마다 화요일에 발송합니다. 수도권은 익일, 그 외 지역은 48시간 이내 도착합니다.' },
  // 연어유·난각분말은 4종 공통 — '해당 원료 자동 제외'는 생선·계란엔 거짓이었다(2026-09-26 사장님 승인 정정).
  { q: '알레르기가 있는 아이도 먹을 수 있나요?', a: '설문에서 닭·오리·돼지·소 가운데 못 먹는 단백질을 고르면 그 레시피는 자동으로 빠져요. 연어유·난각분말은 네 가지 레시피에 모두 아주 조금 들어가는데, 주재료 단백질이 아니라 알레르기를 일으킬 가능성은 매우 낮다고 알려져 있어요. 걱정되시면 카카오톡으로 편하게 문의해 주세요.' },

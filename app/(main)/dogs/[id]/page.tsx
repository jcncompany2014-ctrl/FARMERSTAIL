...
const FOOD_LINE_COLORS: Record<string, string> = {
...
const FOOD_LINE_NAMES: Record<string, string> = {
  basic: 'Basic',
  weight: 'Weight',
  skin: 'Skin',
  premium: 'Premium',
...
export default function DogDetailPage() {
  ...
  useEffect(() => {
    async function loadDog() {
      ...
      if (subs) {
        setSubscriptions(subs as unknown as ActiveSubscription[])
      }
      if (logs) setWeightLogs(logs)
      if (formula) {
        const f = formula as unknown as CurrentFormula
        setCurrentFormula(f)
        // 현재 cycle 의 checkin 응답 여부 — week_2 / week_4 별.
        const { data: checkins } = await supabase
          .from('dog_checkins')
          .select('checkpoint')
          .eq('dog_id', dogId)
          .eq('user_id', user.id)
          .eq('cycle_number', f.cycle_number)
        const status: CheckinStatus = { week_2: false, week_4: false }
        for (const c of (checkins ?? []) as Array<{
          checkpoint: 'week_2' | 'week_4'
        }>) {
          status[c.checkpoint] = true
        }

...
export async function GET(req: Request) {
  ...
  const { data: candidates, error: fetchErr } = await supabase
    ...
    //   만든다**(:266, :572). created_at 으로 고르면 번호 충돌 위험이 있어 번호
    //   산술의 기준은 회차여야 한다.
    //   ✅2026-08-16 데이터 정리 완료: 시간순이 아니던 잔재 2행(v3 이전 cycle 2)을
    //   삭제했고, compute 가 created_at 을 더 이상 덮어쓰지 않아(재계산 시각은
    //   computed_at 분리) 회차 번호와 생성 시간이 다시 갈라질 경로가 없다 —
    //   마이그레이션 20260816000000 + 이 크론과 created_at 정렬 화면들이 이제
    //   같은 처방을 가리킨다(모순 강아지 0 검산).

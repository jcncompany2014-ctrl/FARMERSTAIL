 ...
 *      ※ 가상계좌는 **지금 우리 흐름에서 생기지 않는다** — 구독 전용이라 결제는
 *        전부 빌링키(카드·토스페이)다. 토스 대시보드 수동 결제 대비 휴면 가드로만
 *        남겨 둔다(webhook 헤더에 같은 설명). 상시 경로로 오해하지 말 것.
 *
 * ★ 2026-07-31 주석 정정 — 예전 4항이 **"DONE일 때만 포인트 적립"** 이었다.
 *   포인트는 2026-07-16 에 **전면 폐기**됐고 적립 코드는 이 파일에 없다(:171
 *   주석이 그 폐기를 이미 적고 있었다 — 헤더만 옛말로 남아 있었다).
 *   위 2-b 의 "가격/포인트 재검증" 표현도 같은 잔재다: 지금 검증하는 것은
 *   `order_items` 합계 대 `total_amount` 이고 포인트는 계산에 없다.
 *
 * # 보호
...
export async function POST(req: Request) {
  ...
  if (items && items.length > 0) {
    ...
    // R82-G2 + 쿠폰 폐지(2026-06): discount_amount 위변조 검증.
    // 쿠폰 시스템 폐기 + 단건(인터랙티브) 체크아웃은 자동할인 대상 아님(구독
    // 자동할인은 subscription-charge cron 경로). 따라서 단건 결제 주문에
    // discount_amount 가 있으면 위변조로 간주하고 거부(정가 결제만 허용).
    const storedDiscount = order.discount_amount ?? 0
    if (storedDiscount > 0) {
      captureBusinessEvent('warning', 'order.payment.unexpected_discount', {
        orderId,
        storedDiscount,
      })
      return NextResponse.json(

 ...
 * 아코디언(JS 없이 동작).
 *
 * # ⚠️ 아래 인라인 문구는 **고객에게 보이지 않는다** (2026-07-30 확인)
 * `faqs` 테이블에 published 행이 24개 있어서 실제 화면은 **전부 DB 문구**다.
 * 여기 배열은 DB 가 비었을 때의 폴백이고, 지금은 사실상 죽은 텍스트다.
 * → **이 파일을 고쳐도 사이트는 안 바뀐다.** 고객 문구를 바꿀 곳은 admin
 *   (또는 `faqs` 테이블)이다. 실제로 이걸 배포 확인용 카나리아로 쓰다가 두 번
 *   잘못 결론냈다(AGENTS.md 규칙 — 카나리아는 양방향으로 검산할 것).
 * 2026-07-30 에 DB 쪽 문구를 정정했다(없는 '상품 변경' 기능 안내 · 화식 양 변경
 * 안내 · '언제든' 금지어 · 마감 기준). 아래 폴백은 그 정정을 반영하지 않았으므로
 * **DB 를 비우면 옛 문구가 되살아난다** — 폴백에 의존하지 말 것.
 *
...
const FALLBACK_GROUPS: Group[] = [
  {
    ...
    items: [
      ...
      {
        q: '영양 균형은 어떻게 맞추나요?',
        a: '모든 레시피는 국내 사료 표시기준에 더해, AAFCO(미국)·FEDIAF(유럽)·NIAS(국립축산과학원) 3중 영양 기준을 교차검증해 38가지 필수 영양소 균형을 맞춰요. 우리 아이 분석 결과에서 단백질·지방·칼슘 등 핵심 영양소가 권장 범위 안에 있는지 게이지로 확인하실 수 있어요. 만성질환이 있는 아이는 설문에 입력해 주시면 해당 질환에 부담이 되는 구성이 자동으로 걸러진 상태로 추천됩니다.',
      },
    ],
  },
  {
    ...
    items: [
      {
        q: '언제 발송되나요?',
        a: '발송은 매주 화요일 하루예요. 맞춤 소량 생산으로 가장 신선한 상태에 보내드리려고 발송일을 화요일 하루로 모았어요. 우리 아이 박스는 2주치라 보통 2주마다 화요일에 받으시게 되고, 건너뛰기·일시정지로 다른 화요일 회차로 옮길 수도 있어요. 주문 마감은 발송 전 일요일이라, 그 주 화요일 발송분에 담기려면 일요일까지 시작하거나 조정하시면 됩니다. 발송 후 수도권은 다음 날, 그 외 지역은 대체로 48시간 이내에 도착하고 도서산간은 하루 정도 더 걸릴 수 있어요.',
      },
      {
        q: '배송비가 따로 있나요?',
      ...
      {
        // ★봉투 라벨 기준(제조일로부터 냉동 180일·해동 후 냉장 3일·재냉동 금지) — 예전 '12개월·냉장 7일'은
        //   라벨과 달랐다(식품 안전, 2026-09-26 사장님 승인 정정). 판매 제품은 전부 냉동이다.
        q: '보관·해동은 어떻게 하나요?',
        a: '파머스테일 화식은 모두 냉동으로 보내드려요. 받으시면 바로 냉동실(-18℃ 이하)에 넣어 주세요. 봉투에 적힌 유통기한(제조일로부터 냉동 180일)까지 보관할 수 있어요.\n\n먹이기 전날, 그날 필요한 만큼만 냉장실로 옮겨 천천히 해동해 주세요. 해동한 뒤에는 냉장 보관하고 3일 안에 먹이고, 다시 얼리지 마세요. 한 번에 2~3일치만 해동하는 걸 권해요.',
      },
    ],
  },
  {
    ...
    items: [
      {
        q: '어떤 결제 수단을 지원하나요?',
        a: '정기배송은 등록하신 신용·체크카드로 자동 결제돼요(토스페이먼츠). 카드를 한 번 등록해 두면 회차마다 별도 결제 없이 자동으로 진행됩니다. 카드 정보는 토스페이먼츠가 안전하게 처리하고 파머스테일 서버에는 저장되지 않아요. 정기배송 특성상 가상계좌·휴대폰결제 같은 1회성 결제 수단은 지원하지 않아요.',
      },
      {
        q: '할인은 어떻게 적용되나요?',
      ...
      },
      {
        ...
        a: '발송일이 매주 화요일 하루로 정해져 있어(신선한 소량 생산을 위해), 결제일만 따로 원하는 날짜로 지정하는 방식은 아니에요. 대신 이번 회차를 건너뛰거나 일시정지하면 다음 배송과 결제가 그만큼 뒤로 밀립니다. 주문 마감은 발송 전 일요일이라, 그때까지 마이페이지 → 정기배송에서 조정하시면 다음 화요일 발송분부터 반영돼요.',
      },
      {
        q: '결제 영수증 / 현금영수증은 어떻게 받나요?',
        a: '정기배송은 카드 자동결제라, 결제할 때마다 신용카드 매출전표(영수증)가 토스페이먼츠에서 자동으로 발급돼요. 마이페이지 → 주문 내역에서 회차별로 확인·다운로드하실 수 있어요. 카드 결제에는 현금영수증이 따로 발급되지 않고, 이 매출전표가 지출 증빙 역할을 합니다. 사업자 세금계산서가 필요하시면 고객센터로 문의해 주세요.',
      },
    ],
  },
  {
    ...
    items: [
      {
        q: '정기배송은 어떻게 시작하나요?',
        a: '2분 설문으로 우리 아이 맞춤 식단을 확인한 뒤 화식 비율(곁들임·반반·완전)을 고르면 시작돼요. 발송은 2주마다 화요일 하루이고, 주문 마감은 발송 전 일요일이에요. 일요일까지 결제수단을 등록하시면 그 주 화요일에 첫 박스가 나가고, 마감 이후에 등록하시면 다음 화요일 발송분에 담겨요. 등록을 마치면 화면에 첫 발송일이 정확한 날짜로 표시되니 그 날짜를 확인해 주세요.',
      },
      {
        q: '정기배송을 변경하거나 일시정지할 수 있나요?',
        a: '정기배송 화면에서 일시정지와 해지를 하실 수 있어요. 다음 결제 전에 멈추시면 그 박스는 나가지 않아요. 위약금이나 최소 약정은 없습니다.',
      },
      {
        // ★2026-08-02 검수 — 옛 답변은 **없는 메뉴**를 안내하고 있었다:
        //   "마이페이지 → 정기배송 → '상품 변경'". 저장소 전체에 그런 화면이 없다
        //   (grep '상품 변경' → 이 문자열과 이 주석뿐). 게다가 "네, 언제든
        //   가능합니다" 로 시작해 놓고 뒤에서 "일요일까지 변경하면" 이라 스스로
        //   어긋났다. DB 쪽은 2026-07-30 에 정정됐는데 이 폴백만 옛 문구로 남아,
        //   DB 조회가 실패하면 되살아나는 상태였다.
        q: '구독 도중 단백질 종류를 바꿀 수 있나요?',
        a: '레시피는 우리 아이의 분석 결과를 바탕으로 저희가 제안해 드려요. 알레르기·체중처럼 아이 정보가 바뀌면 다음 제안에 반영되고, 바꾸실 때 수수료나 위약금은 없어요. 지금 바로 조절하실 수 있는 건 화식 비율(곁들임·반반·완전 화식)이고, 정기배송 화면에서 바꾸실 수 있어요. 박스에 담기는 구성 변경은 주문 마감인 일요일까지 반영하면 다음 박스부터 적용돼요.',
      },
      {
        q: '연휴엔 배송이 어떻게 되나요?',

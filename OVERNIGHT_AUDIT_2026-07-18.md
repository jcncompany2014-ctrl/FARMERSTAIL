...
**[회차24 · 반응형 가로 오버플로]** #1 축. 좁은 폰(375px)에서 가로 스크롤 유발 실코드
(고정폭·긴 문자열 no-wrap). **✅ 클린 — 오버플로 관리 견고**:
- 고정폭 전부 <375px(최대 168px), 단독 오버플로 불가. `minWidth:0` (flex 오버플로
  방지 표준 가드) 8회 사용 = 오버플로 인지·관리 신호.
- 긴 문자열: 이름/이메일 표시 화면(DogDetail·Mypage·AppChrome)에 truncate/break 6곳.
- 가로 스크롤 컨테이너 2곳 = 전부 **의도적**(compare 비교표 overflow-x-auto·MyDogs
  캐러셀 flex scroll).
- `whitespace-nowrap` 이 동적 사용자 콘텐츠(dogName·email)에 걸린 곳 **0** → 강제
  오버플로 없음.
→ 세션 초 "모바일 반응형·safe-area·오버플로 수정"(commit b20c6f4)과 일치. 앱 화면
  가로 오버플로 위험 낮음. (admin 모바일은 이번 세션 드로어化로 별도 해결됨.)

**[회차25 · 에러 처리 / 조용한 실패]** #7 축. fetch 실패 시 사용자에게 안내가 뜨는지,
아니면 조용히 아무 일도 안 일어나는지(최악 UX). **✅ 클린 — 에러 처리 모범적**:
- 빈 catch 2건(PreferencesPanel) = `res.json().catch(()=>null)` **JSON 파싱 방어 폴백**,
  전체 실패의 조용한 삼킴 아님(바깥에서 setError 로 안내).
- ★모범 패턴: `PreferencesPanel.patch()` = 낙관적 업데이트(`prev` 저장→setPrefs) 후
  **HTTP 에러·네트워크 에러 양쪽에서 `setPrefs(prev)` 롤백 + setError** →
  "UI 는 ON 인데 서버는 OFF" 같은 거짓말 버그 없음. finally 로 saving 해제.
- 에러 피드백(toast/setErr) 11개 dogs 파일에 널리 사용. 로드 실패도 안내.
→ 조용한 실패 없음. optimistic-revert 까지 갖춘 견고한 에러 처리. (앱이 실패를
  숨기지 않고 사용자에게 정직하게 알림 — 브랜드 "정직" 톤과 일치.)

**[회차26 · kcal 확정치 롤아웃 정합]** #4 축. 급여량 직결 kcal 밀도(닭115/오리120/
돼지115/소120)가 코드 전반에 옛값(130/150) 잔재 없이 반영됐는지. **✅ 클린 — 정합
+ 과거 주석거짓말도 정정 확인**:
- 값 중앙화: `FOOD_LINE_META[line].kcalPer100g`(정본) → lines·boxPricing 전부 그걸 읽음.
  skuModel profile: 115/120/115/120 (2026-07-11 검정 확정) + 160(보류 연어, deferred:true
  USDA 추정 예외). 테스트(lineGrams)가 값 assert. nutrition.ts AVG=1.175 도 정합.
- **메모리 우려("skuModel 헤더 옛값 130/150 주석 거짓말") = 이미 정정됨**: 헤더가
  현재값 명시 + "2026-07-16 까지 설계값이었으나 주석만 옛것" 정정 이력 + "정본은 언제나
  profile.kcalPer100g" + 연어 160 예외 설명 → **모범적 correction note**(거짓 아님).
→ kcal 확정 반영 완료 실증. 급여량 계산의 핵심 상수가 SSOT·정합·문서화됨.

**[회차27 · 로딩/스켈레톤 상태]** #5 축. 데이터 fetch 중 빈 화면(깜빡임)이 뜨는지,
로딩 표시가 있는지(느린 네트워크 체감 품질). **✅ 클린 — 커버리지 양호(강점)**:
- `loading.tsx` 핵심 라우트에 존재: dashboard·dogs·dogs/[id]·analysis·survey·mypage
  + 루트. → 하위 라우트(health·medications 등)는 Next.js App Router 상속으로 커버.
- 클라이언트 로딩 상태(Skeleton·Spinner·isLoading·animate-pulse·BrandLoader) **29개
  파일**에 사용. Suspense 1곳.
→ fetch 중 빈 화면 위험 낮음. 업그레이드 값은 로딩이 아니라 여전히 #A3(전환급여표)·
  #A5(/search)에 집중. (2바퀴째 확인: 업그레이드 여지가 적다 = 코드 성숙 신호.)

---

## 사장님 지시 처리 (2026-07-18 아침)
- **#A3 ✅ 전환급여표 삭제** — 사장님 "우리는 곁들임/반반/풀화식 고정, 전환급여표
  아예 삭제해". lib/transition-plan.ts + test 삭제(참조 0 재확인). 화식 비율이
  고정이라 "0%→서서히 올리는 전환 스케줄"은 모델에 없음. tsc GREEN.
- **#A9 ✅ 화식 티어 일치** — 사장님 "일치시켜". FRESH_TIERS 3중 로컬 정의
  (OrderClient·PlanClient·RecommendationBox)를 정본 lib/subscription/freshTier 로
  일원화(ratio/key/label/sub/badge/copy/note 단일 스키마). freshTierLabel 도 거기서
  파생 → 라벨 통일("곁들임"=선택=표시, 옛 "화식 곁들임" 폐기, "화식 비율: 화식 곁들임"
  중복도 해소). 값(30/60/100) 불변. verify GREEN.
- **#A2 ⏸ 사장님 확인 대기** — skip = 배송 주기 변경 아니라 "다음 배송 미루기"
  (주기는 2주 고정 맞음). 앱=2주/웹=4주 불일치. **웹도 2주로 통일할지 확인 대기**
  (결제 인접이라 무단 수정 X).
- **#A5 ⏸ 사장님 확인 대기** — /search = 앱 내 강아지·다이어리 검색(상품검색은
  구독전환 때 제거됨). 진입점 0. 검색 아이콘 달아 살릴지 / 삭제할지 확인 대기.

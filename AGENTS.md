...
## 8) 크론·웹훅에서 쓰는 헬퍼는 **쿠키 클라이언트를 쓰면 안 된다**

`lib/push.ts` 가 `createClient()`(쿠키) 를 쓰고 있었다. 크론에는 로그인 쿠키가
없으니 `auth.uid()` 가 NULL 이고, 푸시 관련 4개 테이블은 전부 self-only RLS 라서
**모든 조회가 0행**이 됐다. 그런데 코드가 0행을 "받는 사람이 없음"으로 읽어
`{ ok: true, sent: 0 }` 을 돌려주는 바람에 **모든 크론이 성공으로 집계**됐다.
배송·결제·건강 알림 전부가 나가지 않는데 지표는 초록이었다.

같은 파일에서 함께 드러난 것들:
- `push_log` 에는 INSERT 정책이 아예 없었다(SELECT·UPDATE 만) → 기록도 거부.
  그 오류를 감싼 `try/catch` 는 아무것도 잡지 못했다 — `.insert()` 는 throw 가
  아니라 `{ error }` 를 돌려주기 때문이다(규칙1 과 같은 뿌리).
- 웹 푸시 0건일 때 `return` 하는 조기 종료가 **네이티브 팬아웃 앞에** 있어서,
  앱스토어로 설치한 사용자(APNs/FCM 토큰만 있음)에겐 한 건도 안 갔다.
- 광고 표기 `[광고]` 가 웹에만 붙고 네이티브엔 안 붙었다(정보통신망법 §50④).

→ **`userId` 를 인자로 받는 서버 헬퍼는 service_role 로 조회하고 범위는 코드가
`.eq('user_id', ...)` 로 책임진다.** RLS 에 범위를 맡기면 크론에서 무너진다.

## 9) 알림을 보내는 크론은 조용시간(KST 22–08) 밖에 둔다

조용시간 기본값은 설정 화면 토글이 넣는 22→8 이다. 위 8번을 고쳐 알림이 실제로
나가게 되면, 조용시간에 예약된 크론은 그 즉시 **정당하게 차단**된다 — "고객이
껐으니 안 보낸 것"이 되어 버그가 사양으로 위장한다.
**푸시 수정과 크론 시각 이동은 반드시 같이 한다.** 하나만 고치면 조용한 실패가
정당화되기만 한다. 규칙9 테스트가 이걸 지킨다.

## 10) 검증은 부작용 없는 조회로 한다

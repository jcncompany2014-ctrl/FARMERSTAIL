...
describe('calculateNutrition — pregnancy/lactation gender 게이트', () => {
  ...
  it('암컷 비중성화 lactating — 적용', () => {
    const r = calculateNutrition(
      baseDog({ gender: 'female', neutered: false }),
      baseAnswers({ pregnancyStatus: 'lactating' }),
    )
    // 1.6 × 2.5 = 4.0 (default litter 미입력)
    assert.equal(r.factor, 4.0)
    assert.ok(r.riskFlags.includes('LACTATING'))
  })
})

describe('calculateNutrition — NRC 2006 임신/수유 정확 수식', () => {
  it('임신 1주차 — RER × 1.3 (초기)', () => {
    const r = calculateNutrition(
      baseDog(),
      baseAnswers({ pregnancyStatus: 'pregnant', pregnancyWeek: 1 }),
    )
    assert.equal(r.factor, 1.6 * 1.3)
  })

  it('임신 7주차 — RER × 1.8 (후기)', () => {
    const r = calculateNutrition(
      baseDog(),
      baseAnswers({ pregnancyStatus: 'pregnant', pregnancyWeek: 7 }),
    )
    assert.equal(r.factor, +(1.6 * 1.8).toFixed(2))
  })

  it('수유 4마리 — RER × (1.5 + 0.7×4) = RER × 4.3 → cap 5.0', () => {
    const r = calculateNutrition(
      baseDog(),
      baseAnswers({ pregnancyStatus: 'lactating', litterSize: 4 }),
    )
    // 1.6 × 4.3 = 6.88
    assert.equal(r.factor, +(1.6 * 4.3).toFixed(2))
  })
  ...
  it('수유 1마리 — RER × 2.2', () => {
    ...
    assert.equal(r.factor, +(1.6 * 2.2).toFixed(2))
  })

  it('수유 10마리 — cap 5.0 (8마리 기준 1.5 + 0.7×8 = 7.1 → 5.0 cap)', () => {
    const r = calculateNutrition(
      baseDog(),
      baseAnswers({ pregnancyStatus: 'lactating', litterSize: 10 }),
    )
    // 1.5 + 0.7 × 8 = 7.1 → cap 5.0
    assert.equal(r.factor, +(1.6 * 5.0).toFixed(2))
  })
})

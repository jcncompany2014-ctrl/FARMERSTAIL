...
export function renderNewsletterVol01(input: {
  ...
  const section = (kicker: string, title: string): string => `
    <div style="margin:24px 0 10px 0;">
      <div style="font-size:10px;color:#B5533A;letter-spacing:0.16em;text-transform:uppercase;font-weight:700;">${escape(kicker)}</div>
      <div style="margin-top:4px;font-size:16px;font-weight:800;color:#1E1A14;letter-spacing:-0.02em;line-height:1.35;">${escape(title)}</div>
    </div>
  `
  ...
  const body = `
    ...
    <p style="margin:10px 0 0 0;font-size:11.5px;color:#7A7A7A;line-height:1.6;">
    ...
    <p style="margin:0 0 10px 0;">
      <strong style="color:#1E1A14;">오션 오메가 믹스</strong> — 노르웨이 연어와
      국내산 광어를 사람이 먹는 등급으로 손질한 화식이에요. 오메가-3 함량이
      높아 피부·털 컨디션이 거친 아이에게 자주 추천돼요.
    </p>
    ${block.callout(
      'gold',
      `<em style="font-style:normal;color:#7A6A3A;">"3주째 먹이고 있는데 털 빠짐이 많이 줄었어요. 아이가 그릇을 비우는 속도가 달라요."</em><br />
       <span style="font-size:11px;color:#7A7A7A;">— 6살 골든리트리버 보호자 후기</span>`,
    )}

    ${block.hr()}

    ${section('Q&A', '보호자 Q&A')}
    <p style="margin:0 0 8px 0;font-weight:700;color:#1E1A14;">
    ...
    <p style="margin:0 0 10px 0;font-size:12px;color:#3a342a;line-height:1.7;">
      1~3일: 기존 사료 75% + 화식 25%<br />
      4~6일: 기존 사료 50% + 화식 50%<br />
      7~9일: 기존 사료 25% + 화식 75%<br />
      10일~: 화식 100%
    </p>
    <p style="margin:0;font-size:11.5px;color:#7A7A7A;line-height:1.6;">
    ...
    <p style="margin:0;">
      Vol. 02 에서는 <strong style="color:#1E1A14;">단백질 알레르기 진단법</strong>

...
export type AiAnalysisContext = {
  ...
  /**
   * 이 아이가 **이미 파머스테일을 먹고 있는가**(2026-07-26 사장님 제보).
   * 새로 등록한 강아지(아직 구독·주문 없음)에게 "잘 따라오고 계세요" 같은
   * 급여 중 전제 표현이 나가던 버그를 막는다. 구독(카드 등록) 또는 결제완료
   * 주문이 하나라도 있으면 true. false 면 프롬프트가 '시작 전' 톤으로 바뀐다.
   */
...
export function buildAnalysisPrompt(ctx: AiAnalysisContext): {
  ...
  const system = [
    ...
    ``,
    `# ⚠️ 절대 규칙`,
    `- **오직 한국어로만 쓰세요. 영어 단어를 절대 섞지 마세요.** 예: "산책"을 "산walk"로 쓰는 것은 절대 금지.`,
    ...
    `  단, 널리 알려진 표준 원칙은 괜찮습니다(예: "간식은 하루 열량의 10% 이내").`,
    ...
    `  구조: ① 안심·격려로 연다 → ② 이 아이에게 지금 필요한 실행 팁을 **딱 하나**`,
    `  ("이번 주는 산책 10분만 늘려보세요" 같은).`,
    `  문제가 있으면 겁주지 말고 "걱정 마세요, 이것만 하면 돼요" 로. 영양소·kcal 나열 금지. 영양제 금지.`,
    ``,
    `- ★★ **급여 단계에 맞게 말하세요** (user 메시지의 '급여 단계' 참고 — 어기면 안 됩니다):`,
    `  · **아직 시작 안 함**: 이 아이는 **파머스테일을 아직 먹지 않습니다.** 그러므로`,
    `    "잘 따라오고 계세요" / "지금처럼만 하시면 돼요" / "저희와 2주 뒤에 같이 확인해요"`,
    `    처럼 **이미 먹이고 있다는 전제의 표현을 절대 쓰지 마세요.** 대신 이 아이의`,
    `    사정을 관찰해 "이런 점을 챙기면 좋아요"로 담담히 안내하고, 시작에 대한 부담을`,
    `    주지 마세요("드셔보세요" 강요 금지). 재측정·전환 안내도 아직 이릅니다.`,
    `  · **급여 중**: 이미 먹이고 있으니 "잘 따라오고 계세요" 같은 격려, "저희와 2주 뒤에`,
    `    같이 확인해요" 같은 함께하는 표현이 자연스럽습니다.`,
    ...
    `  '아직 시작 안 함' 단계면 finalNote 를 "시작하실 때 이렇게 7일에 걸쳐 바꿔주세요" 처럼`,
    `  **앞으로의 안내**로 쓰고, 이미 전환이 끝난 듯한 표현("이제 완전히 넘어오셨어요")은 쓰지 마세요.`,
  ...
  const userLines = [
    ...
    ``,
    // 급여 단계 — 시스템 프롬프트의 '급여 단계에 맞게 말하세요' 규칙이 이걸 읽는다.
    ctx.feedingStarted
      ? `## 급여 단계: **이미 파머스테일을 먹고 있는 아이입니다.**`
      : `## 급여 단계: **아직 파머스테일을 시작하지 않은 아이입니다** — "잘 따라오고 계세요" 같은 급여 중 전제 표현 금지.`,
    ``,
    `## 기본 정보`,
    `- 견종: ${ctx.breed}`,

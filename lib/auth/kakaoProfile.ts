// 카카오 OAuth 메타데이터에서 출생연도·전화번호 추출(방어적 파서).
//
// ★2026-07-23: 카카오가 전화번호 동의항목을 반려(가입 단계 미수집 사유) → 동의
//   항목에서 전화번호 제외. 콜백은 이제 **출생연도만** 쓴다(pickKakaoBirthYear).
//   pickKakaoPhone 은 나중에 "가입 단계 전화번호 수집"으로 전환할 때 재연결하려고
//   보존한다(현재 앱 미사용, 테스트만 유지). 전화번호는 주문/배송 주소폼에서 수집.
//   (당초 2026-07-20 계획은 [전화번호·출생연도] 축소였으나 전화번호가 반려됨.)
//
// ★불확실성: 카카오→Supabase(OIDC)가 값을 어느 키로 싣는지는 스코프 승인 + 실기기
//   로그인 전엔 확정 불가. 알려진 후보 위치를 폭넓게 훑는다 — user_metadata 직속 /
//   kakao_account 중첩. 없으면 null → 호출측 폴백(age-gate·주문폼). 승인 후 실기기
//   에서 실제 키를 확인해 후보만 좁히면 된다(파싱/정규화 로직은 그대로).
 ...
 *
 * ★현재 앱 미사용(2026-07-23 카카오 전화번호 동의항목 반려·제외). 가입 단계
 *   전화번호 수집으로 전환할 때 콜백(app/auth/callback)에서 재연결. 테스트만 유지.
 */

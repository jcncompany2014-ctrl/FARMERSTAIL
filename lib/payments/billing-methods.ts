...
export function billingMethodFlags(): BillingMethodFlags {
  /**
   * ★ 2026-07-31 — 사장님 지시로 **기본 켜짐**.
   *   "걍 켜. 어차피 실 고객들이 보기 전까지는 나만 계속 보면서 운영할 거니까."
   *
   * # 목표 상태 = 켜짐
   * 토스 PG 심사가 끝나면 토스페이 자동결제도 함께 열리는 것이 정상 경로다.
   * 그러니 **켠 채로 출시하는 게 기본**이고, 지금 켜 둔 것은 그 상태로 가는 길이다.
   *
   * # 지금은 아직 상점 설정 전
   * 실기기에서 토스가 "토스페이 자동결제 결제수단 설정이 없습니다" 로 거절한다
   * (2026-07-30). 그래서 **누르면 실패한다** — 사장님만 보는 지금은 괜찮지만
   * 실 고객이 그 상태를 만나면 안 된다.
   *
   * # 출시 전에 할 일 = **끄기가 아니라 눌러 보기**
   *   폰으로 토스페이 등록을 1회 해서 **창이 열리고 빌링키가 남는지** 확인한다.
   *   · 되면 → 그대로 둔다(목표 상태 도달).
   *   · 토스가 끝내 안 켜 줬으면 → 그때만 `NEXT_PUBLIC_TOSSPAY_BILLING=off`.
   *     이건 **비상구**지 계획이 아니다.
   * 확인 자체를 잊는 것이 위험이라, `lib/audit-rules.test.ts` 규칙23 과
   * PAYMENT_REHEARSAL.md §1-B 가 함께 붙잡는다.
   */

...
export const ANALYSIS_COPY = {
  mer_summary: (name: string, kcal: number) =>
    `${name}이의 하루 권장 칼로리는 ${kcal} kcal 정도예요.\n` +
    `평균적인 활동량 기준이고, 산책 시간이나 활동량에 따라 조금 달라질 수 있어요.`,

  sku_recommendation: (name: string, skuG: number) =>
    `${name}이에게 딱 맞는 1팩은 ${skuG}g이에요.\n` +
    `매일 한 팩이면 ${name}이의 하루 영양이 정확히 채워져요. ` +
    `크지도 작지도 않게, ${name}이 맞춤 사이즈 🐾`,
  ...
  mix_default: {
    topper: (name: string) =>
      `${name}이를 위한 부담 없는 영양 보강\n` +
      `🥗 화식 30% (토퍼) + 평소 사료 70%\n\n` +
      `한 끼 영양 보강용으로 가볍게. ${name}이가 더 좋아하는 한 끼가 돼요.`,
  ...
  combo_for_large: (name: string, sku1: number, sku2: number) =>
    `${name}이가 큰 친구네요. 두 팩을 함께 보내드릴게요.\n\n` +
    `📦 ${sku1}g + ${sku2}g = 1일 ${sku1 + sku2}g (${name}이 맞춤)\n` +
  ...
  over_budget: (name: string, recommendedKrw: number) =>
    `💡 ${name}이의 일일 권장량으로는 하루 ${recommendedKrw.toLocaleString()}원 정도면 충분해요!\n\n` +
  ...
  social_proof: (avgKrw: number, userKrw: number, name: string) =>
    `💬 비슷한 체중의 다른 ${name}이들은 평균 하루 ${avgKrw.toLocaleString()}원 정도 사용하고 계세요.\n\n` +
    (userKrw > avgKrw
      ? `${name}이는 ${userKrw.toLocaleString()}원 응답이라 평균 이상 케어 의지가 있으시네요.`
      : `${name}이의 예산도 충분히 합리적이에요.`),

  // 38영양소 가시화 헤더
  nutrition_overview: (name: string) =>
    `${name}이의 하루 영양 38가지 한눈에 보기 🐾\n\n` +
...
export const RECOMMENDATION_COPY = {
  ...
  chronic: {
    ...
    senior: (name: string) =>
      ...
      `EPA/DHA가 풍부한 연어 화식(FT-S03)이 ${name}이에게 잘 맞아요. ` +
      `매일 0.7g 정도의 오메가-3가 자연스럽게 들어가요 💚`,
  },
...
export const ALERT_COPY = {
  ...
  auto_size_change: (name: string, gainG: number, newSizeG: number) =>
    `${name}이가 더 자랐네요! 4주 동안 약 ${gainG}g 늘었어요.\n\n` +
...
export function withDogName(template: string, dogName: string): string {
  return template.replace(/○○/g, dogName)
}

...
import { tierFromStamps, TIERS } from './tiers.ts'
...
describe('만료 — 적립 시점부터 2년', () => {
  ...
  it('2주마다 적립하면 활성 구독자는 만료를 겪을 수 없다', () => {
    ...
    const perYear = 26
    assert.ok(perYear * STAMP_VALIDITY_YEARS > STAMP_CARD_SIZE * 3, '나무(30개)를 만료 전에 도달')
  })
})

describe('도장판 — 10칸이 차면 다음 장으로', () => {
  it('0개 = 1장째 0칸, 10칸 남음', () => {
...
describe('등급 사다리 — 스탬프 개수 기준', () => {
  it('사장님 확정 매핑: 씨앗0 · 새싹1 · 꽃10 · 열매20 · 나무30', () => {
    assert.deepEqual(
  ...
  it('경계값이 정확히 그 등급에 든다', () => {
    assert.equal(tierFromStamps(0), 'seed')
    assert.equal(tierFromStamps(1), 'sprout')
    assert.equal(tierFromStamps(9), 'sprout')
    assert.equal(tierFromStamps(10), 'bloom')
    assert.equal(tierFromStamps(19), 'bloom')
    assert.equal(tierFromStamps(20), 'fruit')
    assert.equal(tierFromStamps(29), 'fruit')
    assert.equal(tierFromStamps(30), 'mate')
    assert.equal(tierFromStamps(999), 'mate')
  })

  it('첫 결제 1회로 새싹이 된다 ("첫 박스 이상" 정의 그대로)', () => {
    assert.equal(tierFromStamps(1), 'sprout')
  })

  it('카드 1장 = 꽃, 2장 = 열매, 3장 = 나무', () => {
    assert.equal(tierFromStamps(STAMP_CARD_SIZE * 1), 'bloom')
    assert.equal(tierFromStamps(STAMP_CARD_SIZE * 2), 'fruit')
    assert.equal(tierFromStamps(STAMP_CARD_SIZE * 3), 'mate')
  })
  ...
  it('음수·소수를 넣어도 씨앗으로 떨어질 뿐 깨지지 않는다', () => {
    assert.equal(tierFromStamps(-3), 'seed')
    assert.equal(tierFromStamps(0.9), 'seed')
    assert.equal(tierFromStamps(1.4), 'sprout')
  })
})

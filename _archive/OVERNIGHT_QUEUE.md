...
## Phase P — 제품·장바구니 탭 대대적 리뉴얼 (사장님 지시 2026-06-11 밤 — 최우선)
> 사장님: "앱내에 장바구니 탭이랑 제품 탭이 디자인이 너무 마음에 안들어.
> 정말 많은 변화가 있어도 되니까 대대적인 리뉴얼이 있어야 할 거 같아."
> ⚠️ 두 탭 모두 web/app 공유 라우트 — 반드시 isApp / variant='app' 분기 쪽만
> 재설계한다. 웹 에디토리얼(serif/white card/rounded-xl) 절대 불변.
> ⚠️ 가격 계산·재고·주문·쿠폰 로직 불변 — 시각·레이아웃·UX 흐름만 바꾼다.
> 오늘 추가한 장바구니 인라인 쿠폰(적용→즉시 할인)은 기능 그대로 유지.
- [ ] P1. 제품 탭 정독 + 리뉴얼 설계 — 현 구성(히어로/필터/카드 그리드) 진단,
      새 레이아웃 결정. 설계 근거를 OVERNIGHT_LOG 에 기록 후 다음 항목에서 실행.
- [ ] P2. 제품 목록 리뉴얼 실행 — CatalogHero/CatalogChrome/CatalogProductCard 등
      variant='app' 쪽 재설계 (카드 위계·필터 손맛·스크롤 리듬).
- [ ] P3. PDP(제품 상세) app variant 리뉴얼 — 갤러리·정보 위계·구매 CTA 손맛.
- [ ] P4. 장바구니 탭 정독 + 리뉴얼 설계 — 현 핸드오프(헤더카드/리스트/업셀/영수증/
      CTA) 진단 + 새 구성 결정, 설계 근거 LOG 기록.
- [ ] P5. 장바구니 리뉴얼 실행 — CartChrome/CartList(app)/CartUpsell/CartReceipt/
      CartStickyCTA 재설계. 쿠폰 인라인 시트·즉시 할인 표시는 그대로 살린다.
- [ ] P6. 두 탭 마감 패스 — v3 토큰 정합·빈상태·진입 연출(ft-stagger)·전환 통일.

...
- [x] B3-rollout-1. **전 페이지 영어헤더 → 쉬운 한글 + Archivo Black 제거**(27파일 일괄): BETA COHORT→베타 테스트 현황, COHORT INSIGHTS→가입 시기별 분석, PRODUCTS→제품 관리, ORDERS→주문 관리, FINANCE→결제 원장 등. admin Archivo_Black 0·영어h1 0.
- [x] B3-rollout-2. **사이드바 메뉴 쉬운말**: Feature Flags→기능 켜기·끄기, Cron 상태→자동작업 상태, Cohort 분석→가입 시기별 분석, 푸시 캠페인→알림 보내기, 포뮬러 승인→레시피 승인 등.
- [ ] B3-rollout-3. 페이지별 카드 중립톤(rounded-2xl 웜→rounded-lg zinc)·지표 StatCard+도움말·표 쉬운설명 (대시보드 패턴을 cohort·beta-cohort·finance·reports·orders 등에). 순차 진행.

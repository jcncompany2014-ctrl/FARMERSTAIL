...
- **R100-4 M (마이그)**: 동일 타임스탬프 4건(현재 사전식 정렬로 안전, 향후 유일 타임스탬프 컨벤션만 주의) — products CHECK·구독 가드는 적용 완료(상단 R100-4)
...
## 🔴 출시 점검 — 검토 필요 (재무/PII 민감, auto-fix 보류) (2026-06-08)

런칭 점검 sweep 중 발견. 결제/포인트/계정삭제 경로라 **사람 검토 후** 적용 권장
(자동 배포 안 함). 각 항목 검증 완료 — false-positive 아님.

### 1. 적립 cap 비원자성 (rewards) — MEDIUM
`lib/rewards/cap.ts` 의 `capAllowance()` 는 현재 적립합을 읽어 잔여 허용량을
계산하고 반환하지만, 이후 핸들러가 `apply_point_delta` RPC 를 호출하기까지
원자적이지 않다. 동시 요청(예: 다른 탭에서 survey-completion + measurement-upgrade)
이 같은 cap type 에 대해 각각 통과해 **연간 cap 초과 적립** 가능.
- 영향: 같은 cap type(survey_completion 등)에서 상한 우회.
- 권장: cap 검사를 `apply_point_delta` 어드바이저리 락 안으로 이동하거나
  `apply_capped_point_delta(referenceType)` 신규 RPC 로 원자화.
- 보류 사유: 포인트=금전. RPC/마이그레이션 변경이라 검토+테스트 필요.

### 2. referral 마일스톤 보상 balance_after 레이스 — MEDIUM
`supabase/migrations/20260507000003_referral_milestones.sql` 의
`issue_referral_milestones()` 는 point_ledger 최신 balance_after 를 읽어
계산 후 INSERT 하는데, `apply_point_delta` 와 달리
`pg_advisory_xact_lock` 보호가 없다. 동시 적립 시 balance_after 오기 가능.
- 권장: 마일스톤 적립도 `apply_point_delta` RPC 를 경유하거나 동일 어드바이저리
  락 적용. **이미 적용된 마이그레이션이므로 신규 마이그레이션으로 교체**.
- 보류 사유: 운영 DB 함수 + 금전. 검토 필요.

### 3. account/delete — subscriptions·orders 익명화 error 미검사 — MEDIUM
`app/api/account/delete/route.ts:214-252` 의 subscriptions(billing_key 등)·
orders(recipient PII) 익명화 update 는 결과 error 를 확인하지 않는다.
deletionOps(보조 테이블)는 R101-E 로 실패 로깅이 있지만 이 두 update 는 없음.
- 영향: update 가 조용히 실패하면 billing_key/수령인 PII 가 남은 채 200 성공
  반환 → PIPA 즉시파기 관점 잔존 위험.
- 권장: 두 update 에 `{ error }` 캡처 + 실패 시 로깅(최소한 deletionOps 패턴과
  동일). 제어흐름 변경(500 반환)은 half-deleted 상태 영향이 있어 신중히.
- 보류 사유: billing_key 결제수단 토큰 경로 + 계정삭제 민감 흐름.

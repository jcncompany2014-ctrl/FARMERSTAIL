...
- **Supabase: 일일 자동 백업 (Pro 플랜 — 2026-08-19 업그레이드, plan=pro 실측).**
  ...
  - ⚠️ **2026-08-19 이전 시점은 복구할 수 없다.** 업그레이드는 소급 복구 지점을
    만들지 않으므로, 7일치가 온전히 쌓이는 건 8/26 부터다.
  - PITR(임의 시점 복구)은 Pro 에 **포함이 아니라 별도 유료 애드온**이며 현재
    미가입 — 복구 단위는 '일일 백업 시점'이다.
- **Vercel**: Git 기반이라 자동 복원 가능. `git revert` 또는 `vercel rollback`

### 수동 백업 (자동 백업이 있어도, 8/19 이전 시점·오프사이트 보관용)
```bash
...
### 복원 (응급)
1순위: **콘솔 복원** — Supabase → Database → Backups → 시점 선택 → Restore
(Pro 플랜이라 화면이 있다. 단 8/19 이전 시점은 목록에 없다.)

2순위: SQL 덤프에서 복원 (오프사이트 백업본)
```bash

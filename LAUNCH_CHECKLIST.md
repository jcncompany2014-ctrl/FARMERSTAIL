...
## 10. 사장님 결정 대기 (2026-07-24 · AUDIT_FINDINGS 이관 후 그 파일 삭제)

이번 세션에 ★★1 분석 카피·BCS 순화·#38 탈퇴 사진삭제·#9/#71 보상문구·v4.0 레시피·카카오 로그인은 **전부 처리 완료**. 아래는 남은 결정건.

| | 항목 | 내용 |
|---|---|---|
| ⬜ | **CS 보상 수단** | 포인트 폐지 후 사과·보상 시 뭘 줄지 미정 → 정하면 CS 템플릿 복구(다음 박스 할인 쿠폰 추천) |
| ⬜ | **표시광고 자문(#24)** | "처방/AI 영양사" 의료·면허 함의. 앱은 순화됨. **특허 청구항의 "처방" 용어 정합 = 변리사+표시광고 자문** (재노출 시) |
| ⬜ | **substantiation(#54/55)** | 농가·HACCP·인증·원산지 클레임. 현재 `/partners` 숨김 → 다시 켤 때 **실계약/실인증 확인 후** 노출 |
| ⬜ | **웹 가이드라인 연도** | 앱은 연도 제거 완료(2026-07-24), 웹(/science 등)은 그대로 = 표면간 불일치. 통일하려면 웹 편집(선택) |
| ⬜ | **/account/subscriptions 앱톤** | 구독관리 화면 웹톤 잔재 정리(선택·저우선) |
| ⬜ | **미연결 기능** | 가족공유·강아지비교·자가도구 = 완성됐으나 진입점 없음 → 활성화 여부(고ROI·저위험) |
